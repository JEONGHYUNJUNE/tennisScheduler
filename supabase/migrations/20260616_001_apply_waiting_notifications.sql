-- 실행 대상: 기존 OT Tennis DB(id, username, display_name, max_participants 컬럼 사용)
-- 실행 방법: Supabase SQL Editor에 이 파일 전체를 붙여넣고 Run.
-- 포함 내용: 대기 신청, 대기 자동 승격, 알림 테이블/트리거, 멤버관리/일정/참석 RLS.
-- 주의: 이 파일은 여러 번 실행해도 되도록 drop/create 또는 create if not exists 중심으로 작성했습니다.
